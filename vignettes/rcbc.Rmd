...
```{r, include = FALSE}
# skip running this vignette during R package checks
# this is useful to reduce package check times on CRAN
is_check <- ("CheckExEnv" %in% search()) || any(c("_R_CHECK_TIMINGS_",
             "_R_CHECK_LICENSE_") %in% names(Sys.getenv()))
knitr::opts_chunk$set(fig.align = "center", eval = !is_check)

# create default status variable for R package checks (used later)
status <- "optimal"
```

```{r, include = FALSE}
# load package for generating vignette
devtools::load_all()
```

## Introduction

The _rcbc_ package provides an interface to the [_CBC_ (COIN-OR branchand cut)](https://projects.coin-or.org/Cbc) solver (Forrest & Lougee-Heimer 2005). Specifically, _CBC_ is an open-source mixed integer programming solver that is developed as part of the [Computational Infrastructure for Operations Research (COIN-OR) project](http://coin-or.org/). By interfacing with the _CBC_ solver, the _rcbc_ package can be used to generate optimal solutions to optimization problems. To showcase this package, we will define an optimization problem and then find an optimal solution for it.
